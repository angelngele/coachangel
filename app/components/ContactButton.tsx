...
//         if (!formData.message.trim()) newErrors.message = "Even if it's just a little note";
//         if (!formData.referralSource.length) newErrors.referralSource = "Select at least one option.";
//         if (formData.linkedInProfile.trim()) {
//             if (!/^https:\/\/(www\.)?linkedin\.com\/.*$/.test(formData.linkedInProfile)) {
...
//                         <legend className="font-semibold text-black pb-0">What are you seeking support with? (Check all that apply) (Required)</legend>
...
//                         {["Just getting started with self-growth", "I've tried on my own, but feel stuck", "I'm progressing and want to go deeper", "I lead others and want to align more personally"].map(option => (
//                             <label key={option} className=" block text-black">
...
//                     <label className=" block text-black">
//                         This is your space. What&#39;s going on? What led you here? Where do you want to go? (Required)
...
//                             className="border p-2 w-full rounded-lg form-input"
//                         />
...
export default function RegisterForm() {
    ...
    const [responseMessage, setResponseMessage] = useState("");
    const [loading, setLoading] = useState(false);
    const [submitted, setSubmitted] = useState(false); // To trigger showing errors on submit
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-green-100">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl mt-20">
                <h2 className="text-center text-2xl font-bold text-green-500 mb-6">Let&#39;s Talk</h2>

                <form onSubmit={handleSubmit} className="space-y-4">
                    <label className="block text-black">
                        First Name (Required)
                        <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                        {submitted && errors.firstName && <p className="text-red-500">{errors.firstName}</p>}
                    </label>

                    <label className="block text-black">
                        Middle Name
                        <input type="text" name="middleName" value={formData.middleName} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                    </label>

                    <label className="block text-black">
                        Last Name (Required)
                        <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                        {submitted && errors.lastName && <p className="text-red-500">{errors.lastName}</p>}
                    </label>

                    <fieldset className="bg-gray-100 p-4 rounded-lg">
                        <legend className="font-semibold text-black pb-0">What are you seeking support with? (Required)</legend>
                        {["Clarity on life direction", "Confidence and mindset", "Career transitions", "Leadership development", "Accountability & habits", "Public speaking & influence"].map(option => (
                            <label key={option} className="block text-black">
                                <input type="checkbox" name="helpNeeded" value={option} checked={formData.helpNeeded.includes(option)} onChange={handleInputChange} className="mr-2" />
                                {option}
                        ...
                        {submitted && errors.helpNeeded && <p className="text-red-500">{errors.helpNeeded}</p>}
                    </fieldset>
                    ...
                    <fieldset className="bg-gray-100 p-4 rounded-lg">
                        ...
                        {submitted && errors.brandStatus && <p className="text-red-500">{errors.brandStatus}</p>}
                    </fieldset>

                    <label className="block text-black">
                        This is your space. What&#39;s going on? (Required)
                        <textarea name="message" value={formData.message} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                        {submitted && errors.message && <p className="text-red-500">{errors.message}</p>}
                    </label>

                    <label className="block text-black">
                        Email (Required)
                        <input type="email" name="email" value={formData.email} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                        {submitted && errors.email && <p className="text-red-500">{errors.email}</p>}
                    </label>

                    <label className="block text-black">
                        LinkedIn Profile
                        <input type="text" name="linkedInProfile" value={formData.linkedInProfile} onChange={handleInputChange} className="border p-2 w-full rounded-lg" />
                        {submitted && errors.linkedInProfile && <p className="text-red-500">{errors.linkedInProfile}</p>}
                    </label>
                    ...
                    <fieldset className="bg-gray-100 p-4 rounded-lg">
                        ...
                        {submitted && errors.referralSource && <p className="text-red-500">{errors.referralSource}</p>}
                    </fieldset>

                    <button type="submit" disabled={loading} className="bg-green-500 text-white p-3 w-full rounded-lg font-bold hover:bg-green-600 transition">
                        {loading ? "Submitting..." : "Register"}
                    </button>
